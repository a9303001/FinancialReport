...
- `COMPANY_NAME` ← 輪替表 `COMPANY_NAME`（代碼／公司名稱）
- `MARKET` ← 輪替表 `MARKET`（市場）
- `COMPANY_FOLDER` ← 輪替表 `COMPANY_FOLDER`（對應 `FinancialReport/` 底下的資料夾名稱）
- `EXTRA_ANALYSIS` ← 輪替表 `EXTRA_ANALYSIS`（額外分析項目，值為「無」時留空）
...
- 執行日期: 5
- `COMPANY_NAME`：3445 RS 科技，及其中國子公司有研硅（688432.SH）
- `MARKET`：日股/中股
