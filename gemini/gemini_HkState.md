...
|:--:|---|:--:|---|
| **1** | 分析中國房地產 | **§5 → 第四章** | 銷售端／開發端分開講，含官方最新月度數據 |
| **2** | 中國房地產**何時開始衰退**、**何時開始復甦** | **§3 → 第二章** | 衰退起點（月份＋事件）＋ 復甦階段判定 ＋ 季度級時點 ＋ 機率 |
| **3** | 對香港 REIT 的影響 | **§4 → 第三章** | 每檔標的 🟢/🟡/🔴 結論 ＋ 每單位數字 ＋ 傳導時間差 |
| **4** | 中國房地產指標 | **§6 → 第五章** | 指標總表（最新值／環比／同比／發布日／判讀） |
| **5** | 一線城市房價**過去 40 個月**趨勢＋**趨勢圖** | **§7 → 第六章** | 40 列資料表 ＋ 摘要表 ＋ mermaid 折線圖 |
| **6** | 本輪來不及分析、下一輪要分析的項目 | **§9 → 第八章** | Backlog 表（含未完成原因、輪次計數、優先級） |
| **7** | 其它重要項目 | **§8 → 第七章** | 輿情、政策防偽、匯率利率、跨境稅、黑天鵝 |
...
| 6 | **每單位化 ⭐**：只要出現絕對金額（億元），**強制**同步換算 `÷ 最新在外流通單位數`，寫成 `HK$X.XXXX/單位`（算法見 §4.4） | 「減值 9 億」沒感覺，「每單位 −HK$0.14」才有感 |
...
## 3. 【項目 2】何時開始衰退、何時開始復甦（→ 結果檔第二章 · **全報告最重要**）

### 3.1 衰退時間軸（**歷史事實，僅在出現新證據時修訂**）
...
## 4. 【項目 3】對香港 REIT 的影響（→ 結果檔第三章 · **買賣結論寫在這裡**）
...
**Step 5**：補齊 §0.3 七項。REIT 財報**先讀本機資料夾**（零延遲、資料完整），不足再上 HKEXnews／公司 IR。所有金額**每單位化**（§4.4）。
