...
| 網站 | 網域 | 常見錯誤 | 建議做法 |
| :--- | :--- | :--- | :--- |
| Reddit | `reddit.com` | 內建 `WebSearch` 回 `400 not accessible to our user agent`；**Firecrawl 也會回「we do not support this site」**；**Bright Data 回 `Residential Failed (bad_endpoint): … not available for immediate residential (no KYC) access mode in accordance with robots.txt`（需完成 KYC）**；Playwright 在部分環境回 `Chromium distribution 'chrome' is not found at /opt/google/chrome/chrome` | **不要照 §2.1 順序逐一試 firecrawl/brightdata/playwright，直接跳到 Apify Reddit Actor**（已驗證最快最準，SOP 見 §2.9）。⚠️ **2026-09-06 實測 Apify 會回 `Monthly usage hard limit exceeded`（帳號月額度用罄）**；此時 Bright Data／Playwright 皆不可用（見左欄），四鏈全斷，只能改用 `firecrawl_search` + `includeDomains: ["reddit.com"]` 撈索引（實測多為 1~2 年前舊貼文，需嚴格檢查日期），或依 §5.4 誠實記錄失敗 |
...
| X (Twitter) | `x.com` / `twitter.com` | Firecrawl `firecrawl_scrape`（含 `proxy: stealth`）固定回 `All scraping engines failed`；Playwright 需環境有 Chromium | ✅ **已找到有效解（2026-09-06 於 7203 Toyota 實測成功）：兩段式抓取** —— ①先用 **Firecrawl `firecrawl_search` 搭配 `site:x.com` 運算子**定位貼文 URL（注意：`includeDomains: ["x.com"]` 參數實測回傳 0 筆，**必須把 `site:` 寫在 query 字串裡**）；②再用 **Bright Data `scrape_as_markdown` 直接抓該貼文 URL，可取得完整原文＋作者＋時間戳＋瀏覽數**（未登入亦可）。此法優於舊做法的索引摘要，可逐字引述並標註真實日期。僅當 ② 也失敗時，才退回索引摘要並標註「⚠️ 為索引摘要、非原始頁面逐字引述」 |
