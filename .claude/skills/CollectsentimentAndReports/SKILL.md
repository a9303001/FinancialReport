...
- **港股**: 香港經濟日報, 雪球（xueqiu.com，⚠️ JS 動態渲染網站，只抓討論、不抓財報，抓取方式見 7.2.1）, moomoo 社區, 東方財富股吧, LIHKG,etc...
...
> [!IMPORTANT]
> ## 🏆 黃金規則（最重要，一句話記住）
> **任何網站，只要用內建工具（`WebFetch` / `read_url` / `WebSearch`）抓不到資料（空白、報錯、被擋、內容只剩選單框架、搜尋工具回報「網域被封鎖 / 爬蟲不可存取」），都不可以馬上放棄，也不可以直接改用 WebSearch 摘要打發 —— 一定要先換一個 MCP 抓取工具（`firecrawl_scrape` 等）再試一次。MCP 也失敗，才可以放棄或改用搜尋摘要。**
>
...
#### 第二階段：MCP 工具（依鏈往下換，每個工具最多 1 次）
| 次數 | 動作 |
| :--- | :--- |
| **第 1 次** | 改用 MCP 工具，優先順序：`firecrawl_scrape` → `brightdata/scrape_as_markdown` → `apify` → `playwright`。 |
| **若這個 MCP 工具「明確拒絕該網站」** | 有些 MCP 工具會對特定網站直接回「we do not support this site」（例如 **Firecrawl 平台級不支援 `reddit.com`**）。這**不算該網站真的抓不到**，要**往鏈中的下一個 MCP 工具**（brightdata → apify → playwright）再試一次，不要停在第一個。 |
| **鏈中所有可用 MCP 工具都失敗** | 才可以**放棄該網站**，記錄在 Phase 5 報告中，嘗試下一個替代來源。 |
