...
version: 0.1.0
Src: https://github.com/terancejiang/SKILL_financial_report_download
---
/goal
...
Input (stock code, year, report type)
  ...
  → Step 1: Search using search_web tool for PDF on stockn.xueqiu.com
...
Use the **search_web** tool with this query pattern:
...
Run the download script (located at `.agents/skills/download-HK-Report/scripts/download_report.py`):

```bash
python .agents/skills/download-HK-Report/scripts/download_report.py \
  --url "<PDF_URL>" \
