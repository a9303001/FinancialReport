...
def main():
    print("Gathering all stock files...")
    ...
    for f in sorted(md_files):
        ...
        if stk:
            code = stk['code']
            if code not in candidates_dict:
                candidates_dict[code] = stk
                
    print(f"Parsed {len(candidates_dict)} unique existing stocks from disk.")
    
    # Audit fixes for TOP 21~40 (Step 2)
    if '8750' in candidates_dict:
        candidates_dict['8750']['d1'] = 22.7
        candidates_dict['8750']['score'] = 77.7
    if '9101' in candidates_dict:
        candidates_dict['9101']['d1'] = 22.8
        candidates_dict['9101']['score'] = 77.4
    if '00005' in candidates_dict:
        candidates_dict['00005']['d1'] = 22.2
        candidates_dict['00005']['score'] = 77.5

    # Merge new stocks (overwrite or add)
    ...
    print(f"Top 100 selected. Rank 100 score: {top100[-1]['score']:.1f}")
    print(f"Dropped stocks count: {len(dropped)}")
    for d in dropped:
        print(f"  Dropped: {d['code']} {d['name']} (Score: {d['score']:.1f})")

    # Clean directory of all individual md files
    for f in md_files:
        if os.path.basename(f) == '0.StkScreenerResult_gemini.md': continue
        try:
            os.remove(f)
    ...
    # Clean temp_round2 directory if exists
    if os.path.exists('StkScreenerResult/temp_round2'):
        shutil.rmtree('StkScreenerResult/temp_round2', ignore_errors=True)
...
- **待刪 csv列（上輪寫入失敗者）**：無（本輪 4 筆皆已成功寫回並驗證）
