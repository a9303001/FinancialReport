...
3. 讀 StkScreenerResult\ 底下「所有 .csv 股票列表」→ 這是本輪唯一的抽樣母體（§1.4）
4. 稽核本輪指定的 20 檔（TOP 1-20 → 21-40 → 41-60 → 61-80 → 81-100，5 輪循環）
5. 算「說明缺口」= 榜單筆數 − 實際存在的個股 md 檔數
   ├ 缺口 > 10 → 本輪 20 檔額度全部拿去補檔，不抽新股，直接跳到第 9 步
   ├ 缺口 1~10 → 先補滿，剩下的額度才去抽新股
   └ 缺口 = 0 → 台 5 + 美 5 + 港 5 + 日 5，共 20 檔
6. 抽樣（一律只從 csv 抽；順序固定：台股 → 美股 → 港股 → 日股，一個市場做完才換下一個）
   每抽一檔就當場逐條驗 §3.1 七道硬性門檻
   ├ 任一道未過 → ①把該檔從來源 csv 刪除並存回原 csv ②不占額度，立刻重抽
   └ 已在榜單上的 → reload 該檔個股 md，重新分析並更新（占額度，不算新入榜）
7. 七道全過 → 立刻算五維度分數 → 立刻寫該檔的個股 md（不可先批量評分事後補寫）
8. 每做完一個市場就存檔一次（含被改動的 csv）
9. 重排 Top 100 → 重新命名檔案 → 刪除所有落榜檔 → 跑一致性自檢
10. 存檔 + 在對話框只回傳 §7 的精簡摘要（禁止貼出完整內容或大表格）
```
...
| 9 | **市佔敘述必須具名**（🔴 最常違反） | 禁止只寫「龍頭」「行業領先」「市場地位卓越」「市佔率居區域前列」。必須指名**哪個產品／服務**、在**哪個市場範圍**、排**第幾名**、市佔**多少 %**、**來源與日期**，並列出主要競爭對手及其市佔率（詳見 §5.1）。違反 = `資料缺漏`，市場地位維度計 0 分並記入「五、待查」 |
| 10 | **落榜檔自動刪除**（🔴 硬性） | 個股若沒進 Top 100 或從 Top 100 掉出，必須**立即刪除**其 `<排名>-<代號>-<公司>.md`。`StkScreenerResult\` 底下**不得殘留**任何非 Top 100 的個股檔 |
| 11 | **抽樣母體鎖定 CSV**（🔴 硬性） | 所有候選標的**只能**來自 `StkScreenerResult\` 底下的 `.csv` 股票列表。**嚴禁**自行發想標的、憑記憶列名單、或去網路排行榜找股票來補抽。csv 抽乾了就是抽乾了，照 §2 步驟 4 第 9 點處理，**不得**用外部標的湊數 |
| 12 | **不合格即從 CSV 刪除**（🔴 硬性） | 抽到的標的只要 §3.1 七道硬性門檻**任一道未過**（含資料查不到、無法驗證、卡邊界值），除了重抽之外，**必須把該列從它來源的那個 csv 移除並存回原檔**，讓它永遠不再被抽到。刪除須記入「二、修正紀錄」與「一、執行摘要」的 csv 淘汰計數 |
| 13 | **抽到榜上股票要重跑分析** | 抽到的標的若已在「三、Top 100 排名總覽」上，**不可跳過**：必須 reload 它的個股 md，用最新數據重新驗門檻、重新評分、重寫 ①~⑦ 並更新總覽。這種檔**占**本輪額度，但算「更新」不算「新入榜」 |

### 1.3 三大初篩門檻（抽樣當下就要驗，記熟這三條）

```
門檻 A｜TTM PE：  5.0 < PE < 15.0     （嚴格不含邊界；PE = 最新收盤價 ÷ 近四季 EPS；EPS ≤ 0 直接不合格）
門檻 B｜營業利益率：最近完整年度 > 10.0%  且  TTM > 10.0%   （兩個都要過）
門檻 C｜ROE：      5 年 ROE 中位數 > 10.0%  且  最新/TTM ROE > 10.0%   （兩個都要過）
```

⚠️ **PE 必須自己算**，不得直接抄網站上沒標日期或口徑不明的 PE 數字。
...
**CSV 寫回規則（🔴 只准做這一種修改）**：
- 唯一允許的寫入動作＝**刪除整列不合格標的**（原則 12）。
- **必須寫回它原本來源的那個 csv**，不可搬到別的檔、不可合併成一個總表、不可新建 csv。
- 必須**保留原檔的表頭、欄位順序、分隔符號、編碼與換行格式**；除了被刪的那列以外，其他列一個字都不能動。
- 刪除前先確認該列確實是被門檻擋下的那一檔（比對股票代號），**嚴禁整批清空或依猜測刪除**。
- 若 csv 為唯讀或寫入失敗 → 不得放棄門檻判定，仍照常重抽，並把「待刪列」記入「五、待查」，下輪重試。
...
graph TD
    ...
    U --> F
    N -->|否| P{三大門檻全過?}
    P -->|否 或 無法驗證| R[從來源 csv 刪除該列並存回原檔 ➜ 重抽，不占額度]
    R --> E
    P -->|是，累積到 5 檔| F[步驟5：產業判定 + 其餘四道門檻 §3]
    F -->|七道全過| G[步驟6：五維度評分 §4 + 立刻寫個股 md §5]
    F -->|未過| H[從來源 csv 刪除該列並存回原檔 ➜ 淘汰不留存 ➜ 數據不足者記入待查 ➜ 重抽補足 5 檔]
    H --> E
...
5. **一律拒絕、從 csv 刪除、並立刻重抽**的情況：
   ...
   - 計算口徑不明
   - 來源互相衝突且尚未釐清
6. **CSV 刪列動作（🔴 每次拒絕都要做，不可累積到最後才處理）**：
   1. 定位該標的所在的來源 csv 與該列（以股票代號比對）。
   2. 刪掉**那一列**，其餘內容、表頭、欄位順序、分隔符、編碼、換行一律不變。
   3. **存回原 csv 檔**（不可另存新檔、不可合併）。
   4. 記一筆到「二、修正紀錄」：`<代號> 從 <csv 檔名> 移除 — 卡在門檻 <第 X 道>`。
   5. 寫入失敗（唯讀 / 鎖定）→ 仍照常拒絕重抽，把待刪列記入「五、待查」，下輪重試。
7. 重複「抽取 → 驗證 → 收下或（刪 csv + 重抽）」，直到該市場正式名單湊滿 5 檔。被拒的**不占**名額。
8. **抽到已在 Top 100 榜上的股票（🔴 必做，不可跳過）**：
   ...
   4. 重驗**未過**門檻 → 該檔踢出 Top 100、刪掉它的個股 md（原則 10）、並從來源 csv 刪除該列（第 6 點）。
...
| :--- | :--- |
| 七道全過 | 進入步驟 6 |
| 三大初篩門檻（PE/營益率/ROE）在此重驗時失效 | 從正式樣本移除 → **從來源 csv 刪除該列並存回原檔**（步驟 4 第 6 點）→ 回步驟 4 重抽，直到該市場仍有 5 檔合格樣本 |
| 其他任一門檻（市佔／財務安全／ADR／OTC）未過 | 淘汰不留存 → **從來源 csv 刪除該列並存回原檔** → 回步驟 4 **重抽補足** 5 檔 |
| 市佔或財報數據不足（等同門檻 1 或 5 未過） | 一樣視為未過：刪 csv + 重抽，並記入「五、待查清單」 |

**步驟 6 — 即時評分與個股詳細撰寫（單檔閉環）**

對七道門檻全過的股票，**立刻**做完這三件事再處理下一檔：
1. 依 §4 公式算五維度分數與總分。
2. 依 §5 模板寫完 `StkScreenerResult\<排名>-<代號>-<公司>.md`（①~⑦ 全部要有）。**榜上重跑的股票**：把既有檔案**整份改寫**成最新內容（不是附加一段新的），並更新「本區塊最後更新」日期。
...
- 「三」與「四」的數據完全一致
- 排名沒有重複
- **本市場所有被拒標的都已從來源 csv 刪除並存回原檔**，且 csv 表頭／欄位／編碼未被破壞
...
5. **三大門檻全樣本稽核**：逐檔複查本輪四市場的 20 檔正式樣本。只要有任一檔不滿足三大門檻，該市場 Checkpoint **不得標記完成**，必須回步驟 4 重抽重驗。
6. **市佔敘述稽核（禁用詞掃描）**：對「三」總覽與全部個股 md 掃描這些詞——
   `龍頭`、`領先`、`領頭`、`卓越`、`居前列`、`名列前茅`、`優秀標的`、`具備高度行業競爭優勢`。
   命中處若沒有同時附上「產品／服務 + 市場範圍 + 名次 + 市佔率 + 來源日期」，**當場改寫成具名敘述**並記入「二、修正紀錄」；本輪改寫筆數寫進「一、執行摘要」。
...
> 🔴 **本節的共同後果（原則 12）**：抽樣標的只要下表**任一道未通過**（含資料查不到、無法驗證、剛好卡邊界值），除了「重抽」之外，還必須**把該列從它來源的那個 csv 移除並存回原檔**，讓它日後不再進候選池。
...
| **6** | **排除普通股 ADR** | 標的不得為普通股 ADR（**ADR 特別股不排除**）；多地上市時只取主要上市地原股 | 是普通股 ADR → 剔除，改抽原股 |
| **7** | **排除 OTC / 場外交易** | 標的須在正規交易所掛牌（TWSE/TPEx、NYSE/NASDAQ、HKEX、TSE），排除 OTC Markets（Pink / OTCQB / OTCQX）等場外標的 | 屬 OTC → 剔除 |
...
- **初篩重抽紀錄**：台 X 次／美 X 次／港 X 次／日 X 次；正式樣本稽核：`✅ 全數嚴格滿足三大初篩門檻`
...
- csv 候選池：讀入 X 個 csv｜本輪移除 X 檔｜剩餘 台X/美X/港X/日X
...
- [ ] 所有被門檻擋下的標的**都已從原來源 csv 刪列並存回原檔**（沒有另存新檔、沒有合併 csv）
- [ ] 被改動的 csv 表頭、欄位順序、分隔符、編碼、換行格式都跟改動前一致，只少了該刪的那幾列
