...
> **個股詳細檔**：`StkScreenerResult/<排名>-<代號>-<公司>.md`（每家 Top 50 公司獨立一檔）
---
...
> ⭐️ **核心目標**：最關鍵產出為 `StkScreenerResult\<排名>-<代號>-<公司>.md` 中每一檔 Top 50 的完整分析區塊（入榜理由 + 優點 + 缺點與風險）。主結果檔「四、個股詳細」提供 Top 50 索引連結與基本摘要。

### 1.2 核心不可違反原則
1. **讀寫路徑規範**：主結果檔路徑為 `d:\FinancialReport\StkScreenerResult\0.StkScreenerResult_gemini.md`，Top 50 個股詳細檔路徑為 `d:\FinancialReport\StkScreenerResult\<排名>-<代號>-<公司>.md`（排名為二位數補零，如 `01-0883-中國海洋石油.md`）。所有讀寫操作一律限定於 `d:\FinancialReport\StkScreenerResult\` 目錄下。
2. **覆蓋防護機制**：讀取結果檔若遇失敗，嚴禁用空骨架覆蓋！必須先確認檔案狀態。
3. **即時查錯重算**：讀取結果檔時，若發現數字、分類或算術不符公式，必須立即在檔案中更正並記錄於「二、修正紀錄」（只保留最新 5 筆）。
4. **一體化撰寫（Atomic Stock Entry）**：通過門檻進入 Top 50 的股票，**必須立即建立/更新 `StkScreenerResult\<排名>-<代號>-<公司>.md` 並完成評分與個股詳細區塊（①~⑦）**，嚴禁先批量評分事後再補。
...
10. **沒進 TOP 50 檔案自動刪除**（🔴 硬性規定）：若某個股未進入 Top 50 或自 Top 50 落榜，必須立即刪除其對應檔名格式為 `<排名>-<代號>-<公司>.md` 的分析檔案，嚴禁在 `StkScreenerResult\` 目錄下殘留非 Top 50 的個股詳細檔案。
...
- **步驟 3 — 處理待補說明（優先於抽新股）**：
  - 計算缺口 =「三」 Top 50 排名筆數 − `StkScreenerResult\` 目錄下實際存在的 Top 50 `<排名>-<代號>-<公司>.md` 檔案數。
...
- **步驟 6 — 即時評分與個股詳細撰寫（單檔閉環）**：
  - 對通過門檻且進入 Top 50 的股票，**立即**執行以下兩項工作（嚴禁批量評分後延遲撰寫）：
    1. 依 §4 公式計算五維度分數與總分。
...
- **步驟 9 — 排名整合、落榜檔清理與一致性自檢**：
  ...
  2. **Top 50 重排序與重新命名**：依總分由高至低重新排列「三、Top 50 排名總覽」，更新名次編號，並對應更新/重命名 `StkScreenerResult\` 下的 `<排名>-<代號>-<公司>.md` 檔名（例如排名變更時更新前綴 `01`~`50`）。
  3. **落榜檔案自動刪除 (🔴 關鍵清理)**：
     - 檢查 `StkScreenerResult\` 目錄下現有的所有 `<排名>-<代號>-<公司>.md` 檔案。
     - **凡不在最新 Top 50 榜單內的 `<排名>-<代號>-<公司>.md` 檔案，必須立即強制刪除**，確保 `StkScreenerResult\` 底下僅存在 Top 50 的個股檔。
  4. **指標檢驗**：計算完成度指標並填入主結果檔「一、執行摘要」：
     - $A$ =「三」排名筆數
     - $B$ = `StkScreenerResult\` 下 Top 50 `<排名>-<代號>-<公司>.md` 檔案數
...
- **步驟 10 — 最終存檔與摘要回報**：
  ...
  - 再次讀取結果檔驗證無損（確認日期為當天、區塊數未減少、大標題 7 個、Top 50 個股檔連結皆有效）。
...
- **個股詳細檔**：Top 50 檔（每家公司獨立存為 `StkScreenerResult\TOP<排名>-<代號>-<公司>.md`，非 Top 50 強制刪除）
...
## 四、個股詳細
> 本區塊按排名順序收錄 Top 50 各檔之獨立個股分析檔案連結與摘要。完整分析內容個別儲存於 `StkScreenerResult\TOP<排名>-<代號>-<公司>.md`。
