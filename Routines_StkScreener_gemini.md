...
6. 從 csv 抽樣（台→美→港→日）→ 當場驗 §3.1 七道門檻 → 未過則刪 csv 列並重抽（§1.4）；已在榜上則重跑（原則 13）
...
| 12 | **不合格即從 CSV 刪除**（🔴 硬性） | §3.1 任一道未過 → 從來源 csv 刪除該列並存回原檔 + 重抽（操作細則見 §1.4 CSV 寫回規則）。刪除須記入「二、修正紀錄」與「一、執行摘要」 |
...
| 門檻 | 通過條件 |
| :--- | :--- |
| **PE** | `5.0 < TTM PE < 15.0`（不含邊界；EPS ≤ 0 視為不符） |
| **營業利益率** | 最近完整年度 **> 10.0%** 且 TTM **> 10.0%** |
| **ROE** | 5 年中位數 **> 10.0%** 且 最新／TTM **> 10.0%** |

> 任一項未達標、無法驗證、資料過期 → 一律拒絕並重抽（原則 7-8）。完整七道門檻見 §3.1。
...
5. **一律拒絕、從 csv 刪除、並立刻重抽**的情況：
   ...
   - 計算口徑不明
   - 來源互相衝突且尚未釐清
6. **CSV 刪列動作（🔴 每次拒絕都要做，不可累積）**：依 §1.4 CSV 寫回規則刪除該列並存回原檔。記一筆到「二、修正紀錄」：`<代號> 從 <csv 檔名> 移除 — 卡在門檻 <第 X 道>`。寫入失敗 → 記入「五、待查」下輪重試。
...
| 任一門檻未過（含市佔／財務安全／ADR／OTC） | 淘汰 → 依原則 12 從 csv 刪列 → 回步驟 4 重抽補足 5 檔 |
| 數據不足（等同門檻 1 或 5 未過） | 視為未過：同上處理，並記入「五、待查清單」 |
...
- 「三」與「四」的數據完全一致
- 排名沒有重複
- 本市場被拒標的都已依 §1.4 CSV 寫回規則刪除並存回，csv 結構完好
