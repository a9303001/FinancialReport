...
### 1.2 核心不可違反原則
1. **讀寫路徑規範**：主結果檔路徑為 `d:\FinancialReport\StkScreenerResult\0.StkScreenerResult_gemini.md`，Top 100 個股詳細檔路徑為 `d:\FinancialReport\StkScreenerResult\<排名>-<代號>-<公司>.md`（排名補零，如 `01-0883-中國海洋石油.md` 或 `100-xxx-xxx.md`）。所有讀寫操作一律限定於 `d:\FinancialReport\StkScreenerResult\` 目錄下。
...
6. **結構嚴格鎖定**：主結果檔必須嚴格維持 **7 個 `## ` 大標題**，不可增減、重複或錯位。
...
- **步驟 9 — 排名整合、落榜檔清理與一致性自檢**：
  ...
  6. **填寫「七、下一輪待辦」**（每輪必填，不得留空）：列出待補說明、下一輪總覽分批稽核區間（如 TOP 21~40）、待改寫之空泛市佔敘述、待驗證項目及未完成市場。
- **步驟 10 — 最終存檔與摘要回報**：
  - 將最終結果寫入 `StkScreenerResult\0.StkScreenerResult_gemini.md`。
  - 再次讀取結果檔驗證無損（確認日期為當天、區塊數未減少、大標題 7 個、Top 100 個股檔連結皆有效）。
...
| **5** | **財務安全指標** | 依產業別適用專屬指標（見 §3.2 表格） | 指標未達標即剔除 |
| **6** | **排除普通股 ADR** | 排除普通股 ADR（但原股合格、或 ADR 特別股如 `PBR.A` 視為合格） | 命中則剔除 |
| **7** | **排除美國 OTC** | 僅接受 NYSE、NASDAQ、NYSE American 上市標的；排除 Pink Sheet / OTC | 命中則剔除 |
> 註：ETF、封閉式基金（CEF）一律排除。
---

### 3.2 產業專屬財務安全門檻（取代門檻 5）
...
主結果檔 `StkScreenerResult\0.StkScreenerResult_gemini.md` **必須且只能包含以下 7 個 `## ` 大標題**：
...
| :-: | :-- | :-- | :-: | :-- | :-: |
| 1 | 0883 中國海洋石油 | [TOP01-0883-中國海洋石油.md](file:///d:/FinancialReport/StkScreenerResult/TOP01-0883-中國海洋石油.md) | 87.7 | 中國海域原油與天然氣探勘開採獨家專營龍頭... | 2026-08-08 |
