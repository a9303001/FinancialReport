...
3. 稽核本輪指定的 20 檔（TOP 1-20 → 21-40 → 41-60 → 61-80 → 81-100，5 輪循環）
4. 算「說明缺口」= 榜單筆數 − 實際存在的個股 md 檔數
   ├ 缺口 > 10 → 本輪 20 檔額度全部拿去補檔，不抽新股，直接跳到第 9 步
   ├ 缺口 1~10 → 先補滿，剩下的額度才去抽新股
   └ 缺口 = 0 → 台 5 + 美 5 + 港 5 + 日 5，共 20 檔
5. 抽樣（順序固定：台股 → 美股 → 港股 → 日股，一個市場做完才換下一個）
   每抽一檔就當場驗三大初篩門檻（PE / 營益率 / ROE），不合格立刻丟掉重抽
6. 通過三大門檻的，再驗 §3 剩下的四道門檻（共七道）
...
| **每市場配額** | **各 5 檔合格樣本** | 「合格」= 已驗證通過三大初篩門檻（見 §1.3） |
...
**步驟 4 — 三大門檻合格抽樣（每市場最終湊滿 5 檔）**

1. 從該市場隨機或主動檢索抽出候選標的。
2. **避開兩種標的**：① 本輪已成為正式樣本的　② 已經在 Top 100 榜上的。
3. 每抽一檔，**立刻**取得最新收盤價、EPS、營業利益率、ROE。自己算：`TTM PE = 最新收盤價 ÷ 近四季 EPS`。
4. 只有在「資料在 5 個交易日內 + EPS > 0 + §1.3 三大門檻全過」時，才收進正式抽樣名單。
5. **一律拒絕並立刻重抽**的情況：
   - 任一門檻未達標（含剛好等於邊界值）
   - EPS ≤ 0
   - 關鍵數據缺漏
   ...
   - 計算口徑不明
   - 來源互相衝突且尚未釐清
6. 重複「抽取 → 驗證 → 收下或拒絕」，直到該市場正式名單湊滿 5 檔。被拒的不占名額。
7. 記錄重抽軌跡：`市場｜候選代號｜被哪道門檻擋下(PE/營益率/ROE)｜資料日期｜拒絕原因`。結果檔只需在「一、執行摘要」寫各市場重抽次數，不要污染 Top 100。
8. 若所有來源都湊不滿 5 檔 → **停掉該市場**並在「五、待查」記錄來源與原因。**不可**用邊界值、過期值或未驗證值硬湊。
...
| :--- | :--- |
| 七道全過 | 進入步驟 6 |
| 三大初篩門檻（PE/營益率/ROE）在此重驗時失效 | 從正式樣本移除，回步驟 4 重抽，直到該市場仍有 5 檔合格樣本 |
| 其他任一門檻未過 | 淘汰，**不補抽遞補** |
| 市佔或財報數據不足 | 記入「五、待查清單」 |
...
1. 依 §4 公式算五維度分數與總分。
2. 依 §5 模板寫完 `StkScreenerResult\<排名>-<代號>-<公司>.md`（①~⑦ 全部要有）。
...
## 一、執行摘要
- **本次抽樣**：台 5／美 5／港 5／日 5（合計 X / 20 檔；僅計入已驗證通過 PE／營益率／ROE 三大門檻的正式樣本）
...
- 初篩稽核：正式樣本 20 / 20 檔（各市場 5 檔）皆通過三大門檻 ✅｜重抽 X 次
