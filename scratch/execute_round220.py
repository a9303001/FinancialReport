...
# Extract positions for all 100 stocks
positions = {}
for r in table_rows:
    rank = int(r[0])
    code = r[1]
...
# Update dates for TOP 41~60 stocks
top_41_60_rows = table_rows[40:60]
for r in top_41_60_rows:
    rank = int(r[0])
    code = r[1]
    pattern = os.path.join(res_dir, f'{rank:02d}-{code}-*.md')
    if rank == 100:
    ...
    if matching:
        with open(matching[0], 'r', encoding='utf-8') as fp:
            doc = fp.read()
        doc = re.sub(r'-\s*\*\*本區塊最後更新\*\*：\d{4}-\d{2}-\d{2}', '- **本區塊最後更新**：2026-08-12', doc)
        with open(matching[0], 'w', encoding='utf-8') as fp:
            fp.write(doc)
...
- **Checkpoint 完成度**：✅台股 ✅美股 ✅港股 ✅日股（第 220 輪全市場抽樣與稽核完成）
- **本次額度使用**：補說明 0 檔｜初篩 20 檔｜門檻通過 20 檔｜深度分析 20 檔（合計 20 / 20，額度滿檔）
- **結果**：新入榜 0 檔｜榜上重跑更新 20 檔｜落榜 0 檔｜修正 1 處（修正第 57 名 NUE 總分驗算算術標註）
...
- **總覽分批稽核進度**：本輪完成 TOP 41~60 共 20 檔數值與算術稽核（第 3 / 5 輪；下輪進入 TOP 61~80）｜發現/修正 1 處算術偏差（NUE 總分驗算算術標註修復，數值與門檻全數符合）"""

sec2_text = """## 二、修正紀錄 (Self-Fix Log)（只保留最新 5 筆）
| # | 標的 | 錯誤內容 | 修正後 | 依據來源 | 修正日期 |
| :-: | :-- | :-- | :-- | :-- | :-- |
| 5 | TOP 41~60 (20檔) | 執行第 220 輪分批稽核，發現第 57 名 NUE 總分驗算算術標註 23.5 誤植 | 經複查修正為 22.5，其餘 19 檔數值與算術 100% 一致 | 0.StkScreenerResult_gemini.md §2 步驟2 | 2026-08-12 |
| 4 | TOP 21~40 (20檔) | 執行第 219 輪分批稽核，核對總覽數值與個股 md 及算術全鏈公式 | 經複查 20 檔數值與算術全數 100% 一致 | 0.StkScreenerResult_gemini.md §2 步驟2 | 2026-08-12 |
| 3 | TOP 1~100 (100檔) | 主結果檔「三、Top 100 排名總覽」之「市佔定位」欄位遭誤植為語法符號 | 從各個股 md 檔 ⑤-1 區塊自動提取具名「產品｜市場｜排名｜市佔率」填回 | Routines_StkScreener_gemini.md §8 | 2026-08-12 |
| 2 | TOP 81~100 (20檔) | 執行第 217 輪分批稽核，核對總覽數值與個股 md 及算術全鏈公式 | 經複查 20 檔數值與算術全數 100% 一致 | 0.StkScreenerResult_gemini.md §2 步驟2 | 2026-08-12 |
| 1 | T (AT&T) | 抽樣驗證卡在 G5 (負債比率 ≥ 70.0%) | 執行 SOP-DEL 從 US.csv 移除整列並寫回驗證 | StkScreenerResult\\US.csv | 2026-08-12 |"""

sec6_text = """## 六、下一輪待辦（🔴 每輪必填）
- **待補個股說明**：無（Top 100 個股 md 完成度 100/100）
- **下一輪總覽稽核區間**：TOP 61~80（第 4 / 5 輪；順序 1-20 → 21-40 → 41-60 → 61-80 → 81-100 → 1-20）
- **待改寫的空泛市佔敘述**：無（禁用詞掃描 ✅ 0 處殘留，全數具名）
- **csv 候選池狀況**：剩餘 台 140 / 美 71 / 港 92 / 日 193 檔；候選池充足
- **待刪 csv 列（上輪寫入失敗者）**：無
- **下一輪要做的工作**：執行第 221 輪全市場抽樣與 TOP 61~80 數值與算術稽核
...
print("Round 220 execution completed successfully!")
