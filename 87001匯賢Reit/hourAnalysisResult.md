...
> **更新紀錄與時間戳記：**
> - **本次更新時間**：2026-07-14 22:20（延續 2026-07-14 20:42 版本，同日滾動更新；本輪距上輪約 1.5 小時，屬例行確認輪）
> - **前一輪分析基準**：2026-07-14（20:42 版本）
> - **本輪更新重點（本輪仍為「無重大變化」確認輪，僅股價出現小幅、可量化的變動）**：
>   1. **股價小幅走弱、觸及 52 週低點**：多平台（Investing.com、Google Finance、Futu）交叉查證，2026/7/14 收盤價為 **RMB 0.38**，較前一交易日 0.39 元下跌 **-2.60%**（尚未達 3% 顯著變動門檻，但已觸及 52 週低點區間 0.37–0.60 元）。成交量約 460 萬單位，相對 65.23 億流通單位仍屬極低周轉率（約 0.07%），流動性極差問題未改善。Google Finance 技術指標顯示均線呈「強力賣出」訊號，屬技術面雜訊，暫不影響清算價值與 DPU 兩大核心論點。
>   2. **仍無 2026 年中期業績公告**：本輪重新查證 huixianreit.com 官方公告列表，最新公告仍為 **2026/7/2 截至 6/30 之月報表**，中期業績尚未發布；MarketScreener 估計下次財報發布日約為 **2026/8/7**，與歷史慣例互相印證，資料缺口延續、非資料庫延遲所致。
>   3. **土地續期政策：無新增，但補強既有引註來源**：本輪查證確認廣州試點方案發布日為 **2026/5/12**（《廣州市工商業用地使用權續期管理試點方案》），深圳「工業及其他產業用地供應管理辦法」修訂版自 **2026/5/7** 生效（新出讓工業用地上限 30 年、續期經審核後上限 20 年，與先前所述數字一致）；兩者均早於本輪查證窗口，屬既有政策的來源補強，非新政策。中外合作經營／涉外 REIT 底層土地之專門規則仍未出現。
>   4. **本輪社群輿情與新聞掃描均無新增**：雪球（xueqiu.com/S/87001）、東方財富、新浪財經、AAStocks、Futu、Yahoo Finance、Reddit、Seeking Alpha、PTT/Dcard/Mobile01/股市爆料同學會等平台均無與 87001 直接相關之新增內容，反映此檔仍屬低關注度、低流動性標的，與前輪一致。
>   5. **GitHub 本地與遠端資料夾核對**：透過 GitHub API 直接核對 `master` 分支 `87001匯賢Reit/` 資料夾，檔案清單與本地一致（年報x3、中期報告x1、輿情節錄x4、hourAnalysisResult.md），本輪未發現新增或修改檔案，核心基本面數據與 Q1/Q2 結論維持前輪判斷不變。

---

## 📊 最新結論摘要（截至 2026 年 7 月 14 日 22:20）

匯賢產業信託（87001）目前處於極端的**「雙重定價」**狀態，本輪為確認性更新，核心結論不變（股價小幅下滑並觸及 52 週低點，但未改變任何結構性判斷）：
1. **清算價值角度**：北京東方廣場於 2049 年合營期滿清算時，預計每基金單位可回收金額約 **RMB 0.94 至 1.34 元**，相比目前 RMB 0.38 的股價存在 **147% 以上的超額清算溢價**。
...
| :--- | :---: | :---: | :--- |
| **最新股價** | - | **RMB 0.38 元**（-2.60% 較前日，觸及 52 週低點 0.37–0.60 區間） | Investing.com / Google Finance / Futu（2026/7/14 收盤，成交量約 460 萬單位，周轉率仍僅約 0.07%） |
| **已發行基金單位** | - | **6,523,199,235** | HKEX 月報表（截至2026年6月30日確認無變動，無稀釋風險） |
| **最新總市值** | 約 24.79 億元 | 0.38 元 | 計算值 (股價 × 單位數) |
| **每基金單位資產淨值 (NAV)** | 207.03 億元 | **3.1737 元** | 2025 年年報 |
| **股價淨值比 (P/NAV)** | - | **約 0.120** (折讓約 88%) | 計算值 (0.38 / 3.1737) |
...
| **最新股息殖利率 (Yield，TTM)** | - | **約 1.13%** | 計算值 (0.0043 / 0.38) |
...
3.  **剩餘流動資產（60%）**：扣除上述負債後的剩餘境內現金。由於新《外商投資法》實施，自 2020 年起每年年均約 RMB 3.44 億元（每基金單位約 0.0527 元）的折舊與攤銷無法以資本返還渠道匯出，且 2025 年起提留的 10% 法定公積金，均將積累於境內合營公司。至 2049 年，此部分積累的流動資產預計相當龐大，保守估計港方可獲分配 **RMB 15 億元**。
...
*   **結論**：相較於目前 **RMB 0.38** 的股價，不論何種情境，均有 **147% 至 253%** 的清算溢價（本輪股價微幅下滑至 52 週低點，溢價比例較前輪略為擴大）。

### 4. 對估值的影響
*   **對 NAV 的影響**：由於東方廣場固定資產價值將在 2049 年歸零，其會計折舊（每年約 RMB 3.18 億元，每基金單位約 0.0487 元）與公允價值減值（2025年為 -12.91 億元，每基金單位約 -0.1979 元）是必然的趨勢。這導致 NAV 將以年均 3-5% 的速度持續衰退，直至合營期結束。
...
*   **本輪查證：本期無新增地方政策公告，僅補強既有引註的來源日期**。廣州《廣州市工商業用地使用權續期管理試點方案》確認發布日為 **2026/5/12**；深圳《深圳市工業及其他產業用地供應管理辦法》修訂版確認自 **2026/5/7** 生效（新出讓工業用地上限 30 年、續期經審核後上限 20 年，與先前所述數字一致）。以「土地續期 東方廣場 87001」為關鍵字搜尋近一週新聞，未發現新的地方細則發布。
...
2.  **10% 法定公積金新規（財資〔2025〕101號）**：2025 年起實施的財務新規要求外商投資企業需先按稅後利潤的 10% 提取法定公積金，直到累計達到註冊資本 50%（約 22 億元人民幣，每基金單位約 0.3373 元），進一步壓縮利潤。
...
### 🟢 投資利多 (成長動能)
1.  **酒店物業組合強勁復甦**：2025 年酒店物業 NPI 達 RMB 1.04 億元（每基金單位約 0.0159 元，按年增長 19.6%），已重返 2019 年疫前水平。免簽國家擴大將持續利好東方君悅酒店。
...
3.  **流動性折價螺旋**：人民幣計價的港股交易極為冷清，缺乏機構投資者接入，導致股價長期受壓，無法反映其實質資產價值。本輪股價再創 52 週新低（RMB 0.38），成交量僅約 460 萬單位（周轉率約 0.07%），社群輿情掃描也再次確認此檔關注度極低。
...
*   **本輪掃描**：雪球、東方財富、新浪財經、PTT、Dcard、Mobile01、股市爆料同學會、Reddit、Seeking Alpha 等平台本輪均未見與87001直接相關之新討論串，關注度維持前輪的低迷水準，無新增輿情訊號。
...
*   **本輪網路搜尋與比對（2026-07-14 22:20）**：
    - 股價與市值：Investing.com、Google Finance、Futunn、Yahoo Finance 交叉查證，2026/7/14 收盤價 RMB 0.38（-2.60% 較前日），觸及 52 週低點區間（0.37–0.60），成交量約 460 萬單位；Bitget 快取頁面顯示 22:21 的 0.39 元/+1.32% 報價經比對為港股現貨市場收盤（16:00 HKT）後之失效快取，不採用。
    - 官方公告清單：本輪再次查閱 huixianreit.com/eng/investor/ 與 cht 版公告列表 — 確認最新申報仍為 2026/7/2 月報表，尚無中期業績公告；亦交叉確認 FY2025 業績於 2026/3/17 董事會通過（營收 RMB 22.09 億，YoY -5.5%）、2026/4/27 貸款償還公告、2026/4/16 AGM 通函與 2026/5/22 投票結果（一般性授權，無實際回購或單位數變動證據）等既有事件時序一致。
    - 中期業績預估發布日：第三方數據平台（MarketScreener）估計下次財報約 2026/8/7 公布，與歷史慣例（2025 年中期業績於 2025/8/28 公布）推估區間吻合，互相印證。
    - 土地政策：確認廣州試點方案（2026/5/12）與深圳工業用地供應辦法修訂版（2026/5/7 生效）之官方發布/生效日期，數字與既有引註一致。
    - 社群輿情與新聞掃描：雪球（xueqiu.com/S/87001）、東方財富、新浪財經、AAStocks、Futu 新聞頁、Yahoo Finance、Reddit、Seeking Alpha、PTT/Dcard/Mobile01/股市爆料同學會，本輪均無新增與 87001 直接相關之內容。
*   **本輪查證但未發現新增本地資料**：透過 GitHub API 直接核對遠端 `master` 分支 `87001匯賢Reit/` 資料夾檔案清單（年報x3、中期報告x1、輿情/雪球節錄x4：2026-00-12.md、2026-02-11.md、2026-06-16.md、Analysis-stock-report_20260615.md），與本地檔案一致，本輪未發現新增或更新檔案。
*   **資料缺口說明**：2026年中期業績尚未公布（預期8月上旬，第三方估計約8/7），故2026H1實際DPU、期中派息確切金額、以及貸款償還匯兌虧損對可分派金額的最終影響，均待中期業績公告後方能驗證，非資料庫延遲所致。

---

## 🔭 下一輪待辦與觀察重點
1. **中期業績公告**（預期最快 2026/8/7 起）：一旦公布，優先驗證 2026H1 DPU 是否真趨近於零、匯兌虧損實際金額、以及管理層是否重申「一次性事件、不影響全年展望」。
2. **股價技術面**：本輪股價觸及 52 週低點且技術指標轉弱，下一輪需確認是否延續下跌或止穩，並留意是否有機構/ 南向資金介入跡象。
3. **土地續期政策**：持續追蹤北京試點名單、廣州/深圳細則後續擴散情況，特別關注是否有政策明確納入涉外/中外合作經營項目（此為潛在重大利多觸發點）。
4. **REITs 納入互聯互通（南向資金）政策**：市場公認的流動性困局唯一催化劑，下一輪應主動搜尋是否有新進展或官方表態。
5. **五年 ROE / 殖利率完整年度序列**：持續嘗試補齊逐年數據（目前僅有五年中位數），提升 CAGR 與波動度分析的完整度。
